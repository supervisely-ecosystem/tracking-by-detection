...
# pedestrian=0, bicycle=1, car=2, motorcycle=3, bus=5, truck=7
# 'rider' ≈ motorcycle (3), 'other vehicle' — по ситуации 
...
def image_demo(predictor, vis_folder, current_time, args):
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            ...
            print(outputs[:, 6])  # Классы
            print(outputs[:, 4])  # Объекты: score
            # доработка
            # mask = np.isin(outputs[:, 6].astype(int), list(KEEP_CLASSES))
